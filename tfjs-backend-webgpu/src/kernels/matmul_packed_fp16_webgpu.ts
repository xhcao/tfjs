...
export function makeMatMulPackedSource(workPerThread: number[]): string {
  return `
    ...
    shared float16_t mm_Asub[TileAOuter][TileInner];
    shared float16_t mm_Bsub[TileInner][TileBOuter];
...
export class MatMulPackedProgram implements WebGPUProgram {
  ...
  constructor(
    ...
    if (transposeA === false) {
      sampleA = fitA ?
          ...
          `coordsInBounds(ivec2(row, col), ivec2(dimAOuter, dimInner)) ?
            A[batch * ${batchASize} + row * dimInner + col] : float16_t(0)`;
    } else {
      sampleA = fitA ?
          `A[batch * ${batchASize} + col * dimAOuter + row]` :
          `coordsInBounds(ivec2(row, col), ivec2(dimAOuter, dimInner)) ?
            A[batch* ${batchASize} + col * dimAOuter + row] : float16_t(0)`;
    }
    ...
    if (transposeB === false) {
      sampleB = fitB ?
          ...
          `coordsInBounds(ivec2(row, col), ivec2(dimInner, dimBOuter)) ?
            B[batch * ${batchBSize} + row * dimBOuter + col] : float16_t(0)`;
    } else {
      sampleB = fitB ?
          `B[batch * ${batchBSize} + col * dimInner + row]` :
          `coordsInBounds(ivec2(row, col), ivec2(dimInner, dimBOuter)) ?
            B[batch * ${batchBSize} + col * dimInner + row] : float16_t(0)`;
    }
    ...
    this.userCode = `
      ...
      float16_t mm_readA(int row, int col) {
        return ${sampleA};
      }

      float16_t mm_readB(int row, int col) {
        return ${sampleB};
      }

      void mm_write(int row, int col, float16_t value) {
