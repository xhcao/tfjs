...
export class Conv2DMMFp16Program implements WebGPUProgram {
  outputShape: number[];
  shaderKey: string;
  userCode: string;
  ...
  constructor(
    ...
    if (activation) {
      if (hasPreluActivationWeights) {
        activationSnippet = `float16_t activation(float16_t a, ivec4 outCoord) {
            float16_t b = getPreluActivationWeightsAtOutCoords(outCoord);
              ${activation}
            }`;
      } else {
        activationSnippet = `
              float16_t activation(float16_t a, ivec4 outCoord) {
                ${activation}
              }
            `;
      }
