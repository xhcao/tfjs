...
export class BinaryOpSharedFp16Program implements WebGPUProgram {
  outputShape: number[];
  shaderKey: string;
  userCode: string;
  ...
  constructor(
    ...
    const accessDataSnippet = useSharedMemoryWithB ?
        `float16_t a = getAAtOutCoords(coords);
         float16_t b = sharedBuf[${sharedIndexSnippet}];` :
        `float16_t a = sharedBuf[${sharedIndexSnippet}];
         float16_t b = getBAtOutCoords(coords);`;
    ...
    this.userCode = `
        float16_t binaryOperation(float16_t a, float16_t b) {
          ${op}
        }

        shared float16_t sharedBuf[${lastDimensionSize}];
        void main() {
