...
export class DepthwiseConv2DProgram implements WebGPUProgram {
  ...
  constructor(convInfo: backend_util.Conv2DInfo) {
    ...
    this.userCode = `
      void writeResult(int batch, int row, int col, int chan, float16_t value) {
      ...
      void main() {
        ...
        float16_t dotProd = float16_t(0.0);
        ...
        for (int wR = 0; wR < filterDims[0]; wR++) {
          ...
          for (int wC = 0; wC < filterDims[1]; wC++) {
            ...
            float16_t xVal = getX(batch, xR, xC, d1);
            float16_t wVal = getW(wR, wC, d1, q);
            dotProd += xVal * wVal;
          }
        }
